...
public class DatagramPacketDecoder extends MessageToMessageDecoder<DatagramPacket> {
    ...
    public DatagramPacketDecoder(Map<Byte, ? extends PacketParser> packetParsers) {
        this(packetParsers, DEFAULT_PACKET_MAX_DELAY);
    }

    public DatagramPacketDecoder(Map<Byte, ? extends PacketParser> packetParsers,
                                 int maxPacketDelay) {

        this.decoder = new StreamPacketDecoder(packetParsers);
        ...
        if((System.currentTimeMillis() - info.timestamp) <= maxPacketDelay&&
            ...
            try {
                final ByteBuf compositeBuf = Unpooled.wrappedBuffer(
                        info.fragments.values().toArray(new ByteBuf[info.fragments.size()]));
